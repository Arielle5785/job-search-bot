...
def fetch_startup_for_startup() -> list:
    ...
    BASE_URL = "https://startupforstartup.com"
    jobs     = []

    for term in ["customer success", "customer success manager"]:
        url  = f"{BASE_URL}/jobs/?query={term.replace(' ', '%20')}"
    ...
    # Deduplicate within source before returning
    seen, unique = set(), []
    for j in jobs:
        k = make_job_id(j)
        if k not in seen:
...
def fetch_lastartup() -> list:
    SOURCE   = "LaStartup"
    BASE_URL = "https://lastartup.co.il"
    soup     = safe_get(f"{BASE_URL}/jobs/?search=customer+success", SOURCE)
    ...
    jobs = extract_jobs_generic(
        ...
        company_selectors=["[class*='company']", "[class*='employer']", "[class*='startup']"],
    )

    time.sleep(REQUEST_DELAY)
    print(f"  {SOURCE}: {len(jobs)} listings")
    return jobs
...
def fetch_nefesh_bnefesh() -> list:
    ...
    jobs = extract_jobs_generic(
        ...
        company_selectors=[".company-name", "[class*='company']"],
    )
...
def fetch_jobshop() -> list:
    SOURCE   = "JobShop"
    BASE_URL = "https://www.jobshop.co.il"
    soup     = safe_get(f"{BASE_URL}/positions?search=customer+success", SOURCE)
    ...
    jobs = extract_jobs_generic(
        soup, SOURCE, BASE_URL,
        card_selectors=[
            "div.position", "li.position",
            "div[class*='job']", "div[class*='position']", "article",
        ],
        title_selectors=["h2", "h3", ".position-title", "[class*='title']"],
...
def fetch_linkedin() -> list:
    ...
    The other 4 sources run regardless of whether this succeeds.
    """
    SOURCE = "LinkedIn"
    jobs   = []
